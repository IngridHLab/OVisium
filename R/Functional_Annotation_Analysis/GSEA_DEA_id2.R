...
plot_GSEA_id2 <- function(file, anno, assay, dir, ...) {
  ...
  for (c in seq_along(levels(common$id2))) {
    #' Prepare a gene list for GESA 
    c.name <- levels(common$id2)[c]
    common.id2 <- subset(common, id2 == c.name) 
    gene <- common.id2$avg_log2FC
    names(gene) <- common.id2$ENTREZID
    ...
    for (x in seq_along(anno.files)) {
      for (y in seq_along(deg.files)) {
          if (y > length(deg.files)) break 
           try(plot_GSEA_id2(file      = deg.files[[y]], 
                         anno      = names[[x]], 
                         assay     = "GSEA",
                         dir       = input.dir,
