...
FindMarkers_filter_combined_v1 <- function(data, feature.list, slot, test, out.name) {
  ...
  for(f in seq_along(feature.list)) {
    for(t in seq_along(test)) {
      ...
      combined.markers.filt.merged <- 
        bind_rows(combined.markers.filt.list[c("0_FTE", "1_FTE", "3_Mix", "2_Stroma", 
                                               "5_Stroma", "6_Stroma", "7_Stroma",
                                               "8_Stroma", "9_Stroma", "10_Stroma", 
                                               "11_Stroma")])
...
FindAllMarkers_filter_combined_v1 <- function(data, slot, feature.list, test, out.name) {
...
FindMarkers_filter_combined_v1(data = data.sub.filt, 
                               slot = "scale.data", # data or scale.data
                               feature.list = feature.list, 
                               test = test,
                               out.name = "harmony_sample_log2SCTcounts1")

FindAllMarkers_filter_combined_v1(data = data.sub.filt, 
                                  slot = "scale.data",
                                  feature.list = feature.list, 
                                  test = test,
                                  out.name = "harmony_sample_log2SCTcounts1")
...
#' load("Variable_features_filt_SCT.RData")
#' no batch correction but subset and scale
FindMarkers_filter_combined_v1(data = data,
                               slot = "data",
                               feature.list = feature.list, 
                               test = test, 
                               out.name = "recorrected")

FindAllMarkers_filter_combined_v1(data = data, 
                                  slot = "data",
                                  feature.list = feature.list, 
                                  test = test,
                                  out.name = "recorrected")
