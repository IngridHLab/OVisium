...
FindMarkers_filter_combined_core <- function(data, feature.list, slot, test, out.name) {
  
  #' Original Seurat Heatmap on all clusters
  clust.col <- 
    c("#FFFF99","#F0027F","#BF5B17","#666666","#1B9E77","#D95F02","#386CB0","black")
               
  names(clust.col) <-c("2_Stroma","5_Stroma", "6_Stroma", "7_Stroma",
                       "8_Stroma", "9_Stroma", "10_Stroma",
                       "11_Stroma")
...
FindAllMarkers_filter_combined_core <- function(data, slot, feature.list, test, out.name) {
...
FindMarkers_filter_combined_core(data = data.sub.filt, 
                               slot = "scale.data", # data or scale.data
                               feature.list = feature.list, 
                               test = test,
                               out.name = "harmony_sample_log2SCTcounts1_Stroma")

FindAllMarkers_filter_combined_core(data = data.sub.filt, 
                                  slot = "scale.data",
                                  feature.list = feature.list, 
                                  test = test,
                                  out.name = "harmony_sample_log2SCTcounts1_Stroma")
