...
features = list("Epithelial" = sc.markers$gene[1:6], 
                ...
                "Immune" = sc.markers$gene[26:40],
                "OC" = sc.markers$gene[41:45])
...
png(file = paste("vlnplot_ref_by_cluster.png", sep = "."), 
...
png(file = paste("vlnplot_ref_by_sample.png", sep = "."), 
...
#' MS4A1, KLRC1 and TPSB2 doesn't express in all individual sample
sc.markers.filt <- sc.markers$gene[c(1:27, 30:34, 36:40)]
Idents(data) <- "sample"
...
for (ftr in seq_along(sc.markers.filt)) {
  ...
  png(file = paste0(paste("deg", id[i], ftr, sc.markers.filt[ftr], sep = "_"), 
  ...
  print(
    DotPlot(data, assay = "SCT", features = sc.markers.filt, idents = id[i], 
