...
plot_GSEA_patient <- function(file, anno, assay, dir, ...) {
  ...
  common$patient <- factor(common$patient)
...
test.ident <- c("MAST")
markers.list.dir <- "FindMarkers/Patient_Pair_core"
    ...
    for (x in seq_along(anno.files)) {
      for (y in seq_along(deg.files)) {
          if (y > length(deg.files)) break 
           try(plot_GSEA_patient(file      = deg.files[[y]], 
                         anno      = names[[x]], 
                         assay     = "GSEA",
                         dir       = input.dir,
